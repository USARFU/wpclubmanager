...
class WPCM_Admin_Post_Types {
	...
	public function __construct() {
		...
		add_filter( 'gettext', array( $this, 'text_replace' ), 10, 3 );
	...
	public function render_club_columns($column) {
		...
		switch ($column) {
			...
			case 'name' :
				...
				echo '<strong>' . ( $post->ID == $default_club ? '<span class="list-table-club-default">' . __( 'Default', 'wp-club-manager' ) . '</span>' : '' ) . '<a class="row-title" href="' . esc_url( $edit_link ) . '">' . esc_html( $title ) . '</a>';
	...
	public function text_replace( $string = '' ) {
		...
		if ( ! empty ( $post_type ) && $post_type == 'wpcm_player' ) {
			if ( 'Scheduled for: <b>%1$s</b>' == $string ) {
				$string = __( 'Joins on: <b>%1$s</b>', 'wp-club-manager' );
			} elseif ( 'Published on: <b>%1$s</b>' == $string ) {
				$string = __( 'Joined on: <b>%1$s</b>', 'wp-club-manager' );
			} elseif ( 'Publish <b>immediately</b>' == $string ) {
				$string = __( 'Joined on: <b>%1$s</b>', 'wp-club-manager' );
			}
		} elseif ( ! empty ( $post_type ) && $post_type == 'wpcm_staff' ) {
			if ( 'Scheduled for: <b>%1$s</b>' == $string ) {
				$string = __( 'Joins on: <b>%1$s</b>', 'wp-club-manager' );
			} elseif ( 'Published on: <b>%1$s</b>' == $string ) {
				$string = __( 'Joined on: <b>%1$s</b>', 'wp-club-manager' );
